...
(`amount === sum(productPrice × productCount)`) — вона НЕ захищає від підробленої ціни:
якщо клієнт міняє `item.price` через localStorage/DOM ще до відправки запиту, і `amount`,
і `productPrice` рахуються з того самого підробленого значення, тож звірка проходить.
Реальний захист вимагає серверного джерела правди по цінах (захардкоджена мапа
товар→ціна в Netlify Function, або запит до Webflow CMS API), якого зараз немає.
**Обов'язково закрити перед продом з реальними платежами**, інакше клієнт технічно може
оплатити замовлення за підроблену занижену ціну.
